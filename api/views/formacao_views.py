from flask_restful import Resource
from api import api
from ..schemas import formacao_schema
from flask import request, make_response, jsonify
from ..entidades import formacao
from ..services import formacao_service

class FormacaoList(Resource):
    # Listando formação com o método - GET
    def get(self):
        formacao = formacao_service.listar_formacoes()
        # Validando e recuperados todos os registros com many=True
        fm = formacao_schema.FormacaoSchema(many=True)
        return make_response(fm.jsonify(formacao), 200)
    
    # Criando o cadastro de curso - POST
    def post(self):
        # Trazendo a validação feita no arquivo schema
        fm = formacao_schema.FormacaoSchema()
        validate = fm.validate(request.json)

        if validate:
            return make_response(jsonify(validate), 400)
        else:
            nome = request.json['nome']
            descricao = request.json['descricao']

            nova_formacao = formacao.Formacao(nome=nome, descricao=descricao)
            resultado = formacao_service.cadastrar_formacao(nova_formacao)
            return make_response(fm.jsonify(resultado), 201)

class FormacaoDetails(Resource):
    # Buscando curso por ID com o método - GET
    def get(self, id):
        formacao = formacao_service.listar_formacao_id(id)
        if formacao is None:
            return make_response(jsonify("Formação não encontrada"), 404)
        
        fm = formacao_schema.FormacaoSchema()
        return make_response(fm.jsonify(formacao), 200)
    
    def put(self, id):
        formacao_bd = formacao_service.listar_formacao_id(id)
        if formacao_bd is None:
            return make_response(jsonify("Formação não encontrada"), 404)
        
        fm = formacao_schema.FormacaoSchema()
        validate = fm.validate(request.json)

        if validate:
            return make_response(jsonify(validate), 400)
        else:
            nome = request.json['nome']
            descricao = request.json['descricao']

            nova_formacao = formacao.Formacao(nome=nome, descricao=descricao)
            formacao_service.atualiza_formacao(formacao_bd, nova_formacao)
            formacao_atualizada = formacao_service.listar_formacao_id(id)
            return make_response(fm.jsonify(formacao_atualizada), 201)
        
    def delete(self, id):
        formacao_bd = formacao_service.listar_formacao_id(id)
        if formacao_bd is None:
            return make_response(jsonify("Formação não encontrada"), 404)
        
        formacao_service.remove_formacao(formacao_bd)
        return make_response(jsonify("Formação excluida com sucesso"), 204)
        



api.add_resource(FormacaoList, '/formacoes')
api.add_resource(FormacaoDetails, '/formacoes/<int:id>')